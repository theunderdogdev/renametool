...
class RenameTool:
    def __init__(self, title) -> None:
        ...
        self.__bool_vars: dict[str, BooleanVar] = {"use_regex": BooleanVar(value=False)}
    ...
    def __create_gui(self):
        ...
        path_entry = Entry(
            ...
            width=75,
            textvariable=self.__str_vars["path"],foreground='#cbd5e1'
        )
        ...
        self.label_frames["path"].grid(row=0, column=0,padx=10,pady=5)
        # path --------------------
        ...
        use_reg_check = Checkbutton(
            self.label_frames["filter"],
            text="Use Regex",
            variable=self.__bool_vars["use_regex"],
            command=self.on_use_regex,
        )
        use_reg_check.pack(padx=10, pady=10, anchor="sw", side="left")
        apply_filter = Button(
            self.label_frames["filter"],
            text="Apply",
        )
        apply_filter.pack(padx=10, pady=5, anchor="se")
        self.label_frames["filter"].grid(row=1, column=0,
             pady=5, padx=10, 
        )
        # filter --------------------
        ...
        add_selected.grid(row=0, column=0, padx=10, pady=10,)
        rename_selected = Button(self.label_frames["options"], text="Rename")
        rename_selected.grid(row=0, column=1, padx=10, pady=10, )
        self.label_frames["options"].grid(row=3, column=0, padx=10, pady=5, sticky='new')
        # options --------------------
    ...
    def on_use_regex(self):
        print(self.__bool_vars["use_regex"].get())
    ...
    def on_path_confirm(self):
        ...
        contents = self._get_path_contents(_path)
